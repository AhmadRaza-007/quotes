...
class WallpaperController extends Controller
{
    ...
    //             $tmpThumbAuto = $tmpDir . DIRECTORY_SEPARATOR . $autoName;

    //             // run ffmpeg
    //             $cmd = "$ffmpegBin -i " . escapeshellarg($tmpFile) . " -ss 00:00:01 -vframes 1 " . escapeshellarg($tmpThumbAuto) . " -y 2>&1";
    //             $output = shell_exec($cmd);

    //             if (file_exists($tmpThumbAuto)) {
    //                 $stream = fopen($tmpThumbAuto, 'r');
    //                 if ($stream !== false) {
    //                     $putOk = Storage::disk('b2')->put('wallpapers/thumbnails/' . $autoName, $stream);
    //                     if (is_resource($stream)) fclose($stream);

    //                     if ($putOk) {
    //                         $thumbnailPath = 'wallpapers/thumbnails/' . $autoName;
    //                         $thumbnailUrl = Storage::disk('b2')->url($thumbnailPath);
    //                     } else {
    //                         Log::error('Failed to upload generated thumbnail to B2.');
    //                     }
    //                 }
    //                 @unlink($tmpThumbAuto);
    //             } else {
    //                 Log::error("Thumbnail not created. FFMPEG output: $output");
    //             }
    //         } else {
    //             Log::warning('ffmpeg binary not found; cannot auto-generate video thumbnail.');
    //         }
    //     }

    //     // cleanup tmp original
    //     @unlink($tmpFile);
